...
fun Calculator(
    ...
    Box(modifier = modifier) {
        Column(
            ...
            Text(
                text = state.firstNumber + " " + (state.operation?.symbol
                    ?: "") + " " + state.secondNumber,
            ...
            Divider(
                color = LightGray, thickness = 1.dp, modifier = Modifier.padding(vertical = 8.dp)
            )
            Row(
                modifier = Modifier.fillMaxWidth(),
                horizontalArrangement = Arrangement.spacedBy(buttonSpacing)
            ) {
                CalculatorButton(symbol = if (!state.isClearBtn) "AC" else "C",
